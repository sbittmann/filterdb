...
function TableIndex(dataDb, indexDb) {
  ...
  if (!dataDb.getBy) {
    dataDb.getBy = getBy.bind(null, dataDb, indexDb);
  }

  if (!dataDb.createIndexStream) {
    dataDb.createIndexStream = createIndexStream.bind(null, dataDb, indexDb);
  }
...
function ensureIndex(db, indexDb, idxName, rebuild = false) {
  ...
  async function addToIndex(dataToIndex) {
    ...
    await indexDb.put([idxName, valueToIndex, dataToIndex.key], dataToIndex.key);
    pf.finish()
  }
...
function getBy(db, indexDb, index, key, options, cb) {
