...
export default class Table extends Events {
    ...
    constructor(meta, metaChange, db, path, dbname, cache, threadPool, token, getUserFn, system) {
        super();
        this.#meta = meta;
    ...
    async find(func, context = {}, token = this.#token) {
        ...
        let after = await this.emitAfter("query", {
            func,
            context,
            data: result,
        });
    ...
    async save(data, token = this.#token) {
        ...
        let after = await this.emitAfter("save", {
            id,
            data: { _id, ...data },
        });
        return id;
