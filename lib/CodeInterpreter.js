        ...
        if (this.#meta.indexes[key] && this.#meta.indexes[key].build) { 
            ...
            if(value === Object(value)) {
                ...
                if(conditionKey === "$not") {
                    return {
                        *[Symbol.iterator]() {
                            let range1 = db.getRange({  
                                start: indexKey(key, dbValues.LO, dbValues.LO)
                                ,end: indexKey(key, conditionValue, dbValues.LO)
                            })
                            ...
                            let range2 = db.getRange({  
                                start: indexKey(key, conditionValue, dbValues.HI)
                                ,end: indexKey(key, dbValues.HI, dbValues.HI)
                            })
                ...
                if(conditionKey === "$gte" || conditionKey === "$gt") {
                    return {
                        *[Symbol.iterator]() {
                            let range = db.getRange({  
                                start: indexKey(key, conditionValue, conditionKey === "$gte"? dbValues.LO : dbValues.HI)
                                ,end: indexKey(key, dbValues.HI, dbValues.HI)
                            })
            ...
            return {
                *[Symbol.iterator]() {
                    let range = db.getRange({  
                        start: indexKey(key, value, dbValues.LO)
                        ,end: indexKey(key, value, dbValues.HI)
                    })
